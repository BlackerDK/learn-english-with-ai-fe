    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Đọc câu mẫu, ghi âm và nhận chấm điểm từ AI. Lưu lịch sử từng lần luyện.</p>
        </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 bg-white rounded-2xl border border-gray-200 w-fit">
              ...
              <div className="space-y-2">
                {dailyPhrases.map((p) => (
                  <button key={p.id} onClick={() => { setSelectedPhrase(p); setEvaluation(null); setTranscribedText(''); setPracticeError(''); }}
                    ...
                    <span className="block font-mono text-gray-900 mb-0.5 line-clamp-2">{p.text}</span>
              ...
              <div className="glass-card p-6 rounded-3xl space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => handleSpeak(selectedPhrase.text)}
                      className="p-2 rounded-xl bg-white hover:bg-gray-50 text-indigo-400 border border-gray-200 cursor-pointer transition-colors" title="Nghe phát âm mẫu">
                      <Volume2 className="h-4 w-4" />
                    </button>
                  </div>
                  <p className="text-2xl font-bold text-gray-900 font-mono leading-relaxed">{selectedPhrase.text}</p>
                  <p className="text-sm text-gray-600">{selectedPhrase.translation}</p>
                </div>

                {/* Record button */}
                <div className="flex flex-col items-center gap-4 border-t border-b border-gray-200 py-8">
                  {isRecording && (
                ...
                {evaluating && (
                  <div className="flex items-center justify-center gap-2 py-4">
                    ...
                    <span className="text-xs text-gray-600">AI đang phân tích phát âm...</span>
                  </div>
                )}

                {/* Evaluation result */}
                {evaluation && (
                  <div className="space-y-4 border-t border-gray-200 pt-5 animate-fade-in">
                    <div className={`flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl border ${accuracyBg(evaluation.accuracy)}`}>
                      <div className="flex items-center gap-4">
                        <div className="h-16 w-16 rounded-full border-4 border-gray-200 flex items-center justify-center relative shrink-0">
                          ...
                          <span className="text-lg font-extrabold text-gray-900">{evaluation.score}</span>
                        </div>
                        <div>
                          <span className="text-xs text-gray-600 block">Điểm phát âm</span>
                      ...
                      <div className="text-sm text-gray-700 font-mono text-center sm:text-right">
                    ...
                    <div className="space-y-2">
                      <span className="text-xs text-gray-600 font-semibold">Phân tích từng từ:</span>
                      <div className="flex flex-wrap gap-2 p-4 bg-white/2 border border-gray-200 rounded-2xl font-mono text-sm">
                    ...
                    <div className="p-4 bg-indigo-500/5 border border-indigo-500/15 rounded-2xl space-y-1">
                      ...
                      <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-wrap">{evaluation.feedback}</p>
                    </div>
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card p-4 rounded-2xl flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Tìm câu..." value={manageSearch}
                ...
                className="w-full bg-white border border-gray-200 rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-indigo-500/50 transition-colors" />
            </div>
            <div className="flex items-center gap-2">
              <Filter className="h-4 w-4 text-gray-500 shrink-0" />
              <select value={manageLevel} onChange={e => setManageLevel(e.target.value)}
                className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-700 focus:outline-none focus:border-indigo-500/50">
          ...
          {loadingPhrases ? (
            ...
            <div className="glass-card p-12 rounded-3xl text-center space-y-3">
              ...
              <p className="text-gray-600 text-sm">Chưa có câu nào. Thêm câu mới hoặc import hàng loạt.</p>
            </div>
          ) : (
              ...
              {phrases.map(p => (
                <div key={p.id} className="glass-card p-4 rounded-2xl space-y-3 flex flex-col justify-between">
                  <div className="space-y-1.5">
                    <p className="font-mono text-gray-900 font-semibold text-sm leading-relaxed">{p.text}</p>
                    {p.translation && <p className="text-xs text-gray-600">{p.translation}</p>}
                    <div className="flex gap-1.5 flex-wrap">
                      ...
                      <span className="px-1.5 py-0.5 rounded bg-white border border-gray-200 text-[9px] text-gray-500">{langLabel[p.language] || p.language}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                    <button onClick={() => handleSpeak(p.text)}
                      className="p-1.5 rounded-lg text-gray-600 hover:text-indigo-400 hover:bg-white transition-colors cursor-pointer" title="Nghe">
                      <Volume2 className="h-4 w-4" />
                    </button>
      ...
      {activeTab === 'history' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">Tổng <span className="text-gray-900 font-semibold">{historyTotal}</span> lần luyện tập</p>
          </div>
          ...
          {loadingHistory ? (
            ...
            <div className="glass-card p-12 rounded-3xl text-center space-y-3">
              ...
              <p className="text-gray-600 text-sm">Chưa có lịch sử luyện tập. Hãy thử ghi âm một câu!</p>
            </div>
          ) : (
            <div className="space-y-3">
              {history.map(h => {
                ...
                return (
                  <div key={h.id} className="glass-card rounded-2xl overflow-hidden border border-gray-200">
                    <div className="p-4 flex items-center justify-between gap-3 cursor-pointer hover:bg-white/2 transition-colors"
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-mono text-gray-900 text-sm font-semibold truncate">{h.phraseText}</p>
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 space-y-3 border-t border-gray-200 pt-3 animate-fade-in">
                        <div className="flex flex-col sm:flex-row gap-4 text-xs">
                          <div>
                            <span className="text-gray-500 block mb-1">Câu mẫu:</span>
                            <span className="font-mono text-gray-900">{h.phraseText}</span>
                          </div>
                          <div>
                            <span className="text-gray-500 block mb-1">Bạn đã nói:</span>
                            <span className="font-mono text-gray-700">"{h.spokenText}"</span>
                          </div>
                        </div>
                        {words.length > 0 && (
                        ...
                        {h.feedback && (
                          <p className="text-xs text-gray-600 leading-relaxed bg-white/2 p-3 rounded-xl border border-gray-200">
          ...
          {historyTotalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              <button onClick={() => setHistoryPage(p => Math.max(1, p-1))} disabled={historyPage === 1}
                className="p-2 rounded-xl bg-white border border-gray-200 text-gray-600 hover:text-gray-900 disabled:opacity-40 cursor-pointer transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-card w-full max-w-lg rounded-2xl border border-gray-200 shadow-2xl animate-zoom-in overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-bold text-gray-900">{editingPhrase ? 'Sửa câu luyện tập' : 'Thêm câu mới'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-1.5 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-white cursor-pointer"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={handleSavePhrase} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs text-gray-600 font-semibold block mb-1">Câu luyện tập *</label>
                <textarea value={form.text} onChange={e => setForm({...form, text: e.target.value})} rows={3}
                  className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-colors resize-none font-mono"
                  placeholder="Nhập câu cần luyện phát âm..." />
              </div>
              <div>
                <label className="text-xs text-gray-600 font-semibold block mb-1">Dịch nghĩa</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-600 font-semibold block mb-1">Cấp độ</label>
                  <select value={form.level} onChange={e => setForm({...form, level: e.target.value})}
                    className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 transition-colors">
                    {LEVELS.map(l => <option key={l} value={l} className="bg-gray-900">{l}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-gray-600 font-semibold block mb-1">Ngôn ngữ</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setIsModalOpen(false)}
                  className="flex-1 bg-white hover:bg-gray-50 text-gray-900 font-medium py-2 px-4 rounded-xl text-sm border border-gray-200 cursor-pointer transition-colors">Hủy</button>
                <button type="submit"
      ...
      {isImportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-card w-full max-w-lg rounded-2xl border border-gray-200 shadow-2xl animate-zoom-in overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-bold text-gray-900">Import hàng loạt</h2>
              <button onClick={() => setIsImportOpen(false)} className="p-1.5 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-white cursor-pointer"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={handleImport} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs text-gray-600 font-semibold block mb-1">Dán danh sách câu *</label>
                <textarea value={importText} onChange={e => setImportText(e.target.value)} rows={8}
                  className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-colors resize-none font-mono"
                  placeholder={"Hello, how are you? | Xin chào, bạn khỏe không?\nCould you say that again? | Bạn có thể nói lại không?"} />
              </div>
              <div>
                <label className="text-xs text-gray-600 font-semibold block mb-1">Cấp độ cho tất cả câu</label>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-card w-full max-w-sm rounded-2xl border border-gray-200 shadow-2xl p-6 space-y-4 animate-zoom-in text-center">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900">Xóa câu luyện tập?</h3>
              <p className="text-xs text-gray-600 mt-1 leading-relaxed">
                Bạn có chắc muốn xóa câu <span className="font-semibold text-gray-900 font-mono">"{deleteText}"</span>?
              </p>
            </div>
            <div className="flex gap-3 pt-2">
              <button onClick={() => setDeleteId(null)}
                className="flex-1 bg-white hover:bg-gray-50 text-gray-900 font-medium py-2 rounded-xl text-sm border border-gray-200 cursor-pointer">Hủy</button>
