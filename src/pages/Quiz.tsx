  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-gray-400 mt-1">Sử dụng Google Gemini để sinh câu hỏi trắc nghiệm kiểm tra trình độ thông minh.</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="glass-card p-6 rounded-3xl md:col-span-1 space-y-4 h-fit">
            <div className="flex items-center gap-3 border-b border-white/5 pb-3">
              ...
              <h2 className="text-sm font-bold text-white">Tạo Quiz mới</h2>
            </div>

            <form onSubmit={handleGenerate} className="space-y-4">
              <div>
                <label className="text-xs text-gray-300 block mb-1 font-semibold">Chủ đề mong muốn</label>
                <input
                  type="text"
                  placeholder="e.g. Du lịch, Ẩn thực, hoặc Trống..."
                  value={topic}
                  onChange={(e) => setTopic(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-colors"
                />
              </div>

              <div>
                <label className="text-xs text-gray-300 block mb-1 font-semibold">Số lượng câu hỏi: {count}</label>
                <input
                  type="range"
                  min="3"
                  max="10"
              ...
              <div className="flex items-center gap-2.5 py-1">
                <input
                  ...
                  className="h-4 w-4 bg-white/5 border border-white/10 text-indigo-500 rounded focus:ring-indigo-500"
                />
                <label htmlFor="vocab" className="text-xs text-gray-300 cursor-pointer select-none">
          ...
          {/* History Panel */}
          <div className="glass-card rounded-3xl md:col-span-2 overflow-hidden border border-white/5 flex flex-col max-h-[50vh] md:max-h-[60vh]">
            <div className="px-6 py-4 border-b border-white/5 bg-white/2 flex items-center gap-3">
              <Clock className="h-5 w-5 text-cyan-400" />
              <h2 className="text-sm font-bold text-white">Lịch sử làm Quiz</h2>
            </div>
                ...
                history.map((h) => (
                  <div key={h.id} className="p-4 flex items-center justify-between text-left text-xs">
                    <div className="space-y-1">
                      <span className="font-bold text-white block truncate max-w-[200px] sm:max-w-xs">{h.topic}</span>
        ...
        <div className="max-w-2xl mx-auto glass-card p-6 md:p-8 rounded-3xl space-y-6">
          ...
          <div className="space-y-3">
            {activeQuestion.options && activeQuestion.options.length > 0 ? (
              ...
              activeQuestion.options.map((opt, idx) => {
                ...
                let buttonStyle = 'bg-white/2 border-white/5 hover:bg-white/5 text-gray-200';
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-indigo-500 transition-colors"
                  disabled={submitted}
                />
                {submitted && (
          ...
          {submitted && (
            <div className="p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl space-y-1 animate-fade-in">
              ...
              <p className="text-xs text-gray-300 leading-relaxed">{activeQuestion.explanation}</p>
            </div>
          )}
          ...
          <div className="flex gap-4 pt-4 border-t border-white/5">
            {!submitted ? (
              ...
              <button
                ...
                className="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-xl text-xs transition-all duration-300 cursor-pointer"
              >
      ...
      {gameState === 'result' && (
        <div className="max-w-md mx-auto text-center space-y-6 py-8 animate-fade-in">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-white">Kết quả bài Quiz</h2>
          ...
          <div className="flex justify-center gap-3">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium py-2.5 px-5 rounded-xl text-sm transition-all duration-300 cursor-pointer"
            >
              Về trang chủ
            </a>
          </div>
